...
export async function GET(request: Request) {
  try {
    ...
    for (let i = 0; i < poolsData.data.length; i += BATCH_SIZE) {
      ...
      const batchResults = await Promise.all(
        batch.map(async (pool: any) => {
          ...
          try {
            ...
            // Fetch both token info and security details
            ...
            // Find the token info that matches the baseTokenId
            const matchingToken = tokenInfoData.data.find(
              (token: any) => token.id === baseTokenId
            );

            // Get latest snapshot from matching token
            ...
            console.error(`Error processing pool ${poolAddress} with baseTokenId ${baseTokenId}:`, error);
            // Return partial data with basic info when there's an error
            return {
              id: pool.id,
              baseTokenId: baseTokenId,
              name: baseToken?.attributes?.name || '',
      ...
      combinedData.push(...batchResults.filter(Boolean));
